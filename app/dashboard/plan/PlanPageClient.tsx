...
import { useGetPlansQuery, useCreateCheckoutSessionMutation, useGetProfileQuery } from "@/lib/authApi";
...
export default function PlanPageClient() {
  const { data: plans, isLoading: isPlansLoading, isError: isPlansError } = useGetPlansQuery();
  const { data: profile } = useGetProfileQuery();
  const [createCheckoutSession, { isLoading: isCheckoutLoading }] = useCreateCheckoutSessionMutation();
  const [loadingPlanId, setLoadingPlanId] = useState<number | null>(null);

  const currentPlanType = profile?.data?.pricing_plan?.plan_type?.toLowerCase() || "";
  ...
  const handleCheckout = async (planId: number) => {
    ...
    try {
      ...
      const errMsg = err?.data?.message || err?.message || "Failed to initiate checkout.";
      toast.error(errMsg);
    } finally {
  ...
  const getPlanTheme = (plantype: string) => {
    ...
    if (isBuilder) {
      return {
        cardBg: "bg-white border-2 border-slate-900 shadow-lg scale-100 lg:scale-[1.03] z-10",
        headerBg: "bg-slate-950 text-white border-b border-slate-900",
        buttonStyle: "bg-[#FD6E6E] hover:bg-[#e05656] text-white shadow-rose-200",
        tag: "Best Value",
    ...
    if (isAnchor) {
      return {
        cardBg: "bg-white border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1",
        headerBg: "bg-gradient-to-r from-amber-500 to-amber-600 text-white border-b border-amber-600",
        buttonStyle: "bg-slate-900 hover:bg-slate-800 text-white shadow-amber-200",
        tag: "Premium",
    ...
    return {
      cardBg: "bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1",
      headerBg: "bg-slate-100/80 text-slate-800 border-b border-slate-100/50",
      buttonStyle: "bg-slate-50 hover:bg-slate-800 hover:text-white border border-slate-200 text-slate-600",
      tag: null,
    ...
    <div className="max-w-7xl mx-auto w-full py-8 px-2 md:px-6 font-sans text-slate-800 flex flex-col gap-8 pb-20">
      ...
      {isPlansLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch w-full py-12">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white rounded-3xl border border-slate-100 p-8 flex flex-col gap-6 animate-pulse shadow-sm">
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 shadow-sm">
          <p className="text-slate-500 font-semibold mb-3">Failed to load membership plans.</p>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 items-stretch w-full">
          {plans?.map((plan) => {
            ...
            const isCurrentPlan = currentPlanType === plan.plantype.toLowerCase();
            ...
            return (
              ...
              >
                {theme.tag && (
                  <div className={`absolute top-4 right-4 text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full shadow-md ${theme.badge}`}>
                    {theme.tag}
                  </div>
                )}

                {/* Header segment */}
                <div className={`p-8 flex flex-col items-center justify-center text-center relative ${theme.headerBg}`}>
                  ...
                  <div className="flex items-center gap-4 justify-center mt-2">
                    <div className="flex items-baseline">
                      ...
                      <span className="text-xs font-semibold opacity-70 ml-1">/Month</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-8 flex flex-col flex-1 gap-6">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wide">Monthly Limit</span>
                  ...
                  <ul className="flex flex-col gap-4 flex-1">
                    {featuresList.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                        <div className={`rounded-full p-1 mt-0.5 shrink-0 ${theme.checkBg}`}>
